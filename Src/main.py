    ...
    def save_s(self):
        ...
        for i in range(len(self.user_ans)):
            if self.user_ans[i]==self.ans_key[i]:
                marks_scored+=1
        test_table = str(self.test_data[0])+"_"+str(self.test_name[2])+"_marks"
        self.cursor_1.execute("INSERT INTO {test_table} VALUES({user_id},\'{user_name}\',{marks},\'{ans_given}\')".format(test_tableuser_id=self.login_userdata[0],user_name=self.login_userdata[1],marks=marks_scored,ans_given=self.user_ans))
